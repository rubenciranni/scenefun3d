# Adapted from https://github.com/ScanNet/ScanNet/blob/master/BenchmarkScripts/util_3d.py
...
from scenefun3d.eval.functionality_segmentation.eval_utils import util
...
def load_ids(filename):
    ...
    # quick and dirty fix for open-vocab inst seg
    # ids[np.logical_and(ids < 1000, ids > 0)] += 1000
    # ids[np.logical_and(ids < 255, ids > 0)] += 1000
    # ids[ids == 255] = 0 # 255001
    # breakpoint()

    # leave the excluded points as 255

    return ids

def get_excluded_point_mask(gt_mask):
    # ids = open(filename).read().splitlines()
    # ids = np.array(ids, dtype=np.int64)
